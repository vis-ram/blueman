...
class _Agent:
    ...
    def _on_action(self, _notification, action):
        ...
        if action == "accept":
            self.transfers[self._transfer.object_path] = {
                'path': self._session.root + '/' + os.path.basename(self._transfer.name),
                'size': self._transfer.size,
                'name': self._name
            }
            self._agent.reply(self.transfers[self._transfer.object_path]['path'])
            self._allowed_devices.append(self._session.address)
            GObject.timeout_add(60000, self._allowed_devices.remove, self._session.address)
        else:
            self._agent.reply(obex.Error.Rejected)

    def _on_authorize(self, _agent, transfer_path):
        self._transfer = obex.Transfer(transfer_path)
        self._session = obex.Session(self._transfer.session)
        try:
            device = Device(self._applet.Manager.get_adapter().find_device(self._session.address))
            self._name = device.Alias
            trusted = device.Trusted
        except Exception as e:
            dprint(e)
            self._name = self._session.address
            trusted = False
        ...
        if self._session.address not in self._allowed_devices and not (self._config['opp-accept'] and trusted):
            self._notification = Notification(_("Incoming file over Bluetooth"),
                _("Incoming file %(0)s from %(1)s") % {"0": "<b>" + self._transfer.name + "</b>",
                                                       "1": "<b>" + self._name + "</b>"},
        ...
        elif self._transfer.size > 350000:
            self._notification = Notification(_("Receiving file"),
                _("Receiving file %(0)s from %(1)s") % {"0": "<b>" + self._transfer.name + "</b>",
                                                        "1": "<b>" + self._name + "</b>"},
                pixbuf=get_icon("blueman", 48), status_icon=status_icon)
